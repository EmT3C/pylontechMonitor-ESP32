...
void MQTTHandler::publishDiscovery() {
  ...
  pub_cfg("soc",          "Battery SoC",        String(MQTT_TOPIC_ROOT) + "soc",          "%",  "battery");
  ...
  pub_cfg("dc_power",     "Battery DC Power",   String(MQTT_TOPIC_ROOT) + "dc_power",     "W",  "power");
  ...
  for (int i = 1; i <= MAX_PYLON_BATTERIES; ++i) {
    ...
    pub_cfg(idp + "_power",    String("Battery ") + i + " Power",    String(MQTT_TOPIC_ROOT) + i + "/power",    "W", "power");
...
void MQTTHandler::publishData() {
  if (!s_client || !s_stack || !s_stack->valid) return;
